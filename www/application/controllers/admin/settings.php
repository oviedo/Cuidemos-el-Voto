...
/**
     * Handles settings for Eflyer SMS Providers 
     */
	function smseflyer()
	{
		$this->template->content = new View('admin/smseflyer');
		$this->template->content->title = 'Settings';
	    ...
	    if ($_POST)
	    {
			...
			$post->add_rules('eflyer_username', 'required', 'length[3,50]');
			$post->add_rules('eflyer_password', 'required', 'length[5,50]');
	        ...
	        if ($post->validate())
	        {
	            // Yes! everything is valid
				$settings = new Settings_Model(1);
				$settings->eflyer_username = $post->eflyer_username;
				$settings->eflyer_password = md5($post->eflyer_password);
				$settings->date_modify = date("Y-m-d H:i:s",time());
				$settings->save();
		...
		else
		{
			...
			$form = array
		    (
				'eflyer_username' => $settings->eflyer_username,
				'eflyer_password' => ''
		    );
		}
